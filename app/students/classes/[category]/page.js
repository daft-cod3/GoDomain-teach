    ...
    <div className="app-shell">
      <div className="dashboard-layout">
        ...
        <main className="dashboard-main space-y-6">
          <section className="glass overflow-hidden rounded-4xl border border-(--border) p-6 shadow-(--shadow)">
            <div
              className="rounded-[28px] p-5 text-foreground"
              style={{
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-(--fg)/55">
                    Class category
                  </p>
                  <h1 className="mt-2 font-display text-4xl font-semibold">
                    {categoryConfig.title}
                  </h1>
                  <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-(--fg)/68">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {categoryConfig.levels.map((level) => (
                  <span
                    key={level}
                    className="rounded-full border border-(--border) bg-(--panel)/82 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em]"
                  >
                    {level}
                  </span>
                ))}
          ...
          <section className="space-y-4">
            ...
            {learners.length > 0
              ...
              : <div className="rounded-[30px] border border-dashed border-(--border) bg-(--panel)/70 px-6 py-14 text-center">
                  <p className="text-sm font-semibold uppercase tracking-[0.22em] text-(--fg)/52">
                    No learners in this category yet
                  </p>
                  <p className="mx-auto mt-3 max-w-xl text-sm font-medium leading-6 text-(--fg)/65">
                  ...
                  <div className="mt-5 flex flex-wrap justify-center gap-2">
                    {classCategoryCatalog.map((categoryItem) => (
                      <Link
                        ...
                        className="rounded-full border border-(--border) bg-(--panel) px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em]"
                      >
                        {categoryItem.id}
                      </Link>
                    ))}
